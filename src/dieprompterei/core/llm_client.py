...
Provides both sync and async clients for different use cases.
"""
...
# ============================================================================
# Async LLM Clients (for parallel execution)
# ============================================================================


class AsyncLLMClient(ABC):
    """Abstract async interface for LLM providers."""
    ...
    async def generate(
        ...
        """
        Generate text response from LLM asynchronously.
...
class AsyncAnthropicClient(AsyncLLMClient):
    """Async Anthropic Claude API client with aiohttp backend."""
...
def create_async_llm_client(config: LLMConfig) -> AsyncLLMClient:
    """
    Factory function to create async LLM client based on config.

    Args:
        config: LLM configuration

    Returns:
        Configured async LLM client

    Raises:
        ValueError: If provider not supported
    """
    if config.provider == "anthropic":
        return AsyncAnthropicClient(model=config.model)
    elif config.provider == "openai":
        return AsyncOpenAIClient(model=config.model)
    else:
        raise ValueError(
            f"Unsupported LLM provider: {config.provider}. "
            f"Supported: anthropic, openai"
